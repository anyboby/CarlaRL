...
class BaseWrapper(object):
    ...
    def __init__(self, world, max_frames=1000, render=False):
        ...
        self._start(VEHICLE_MODELS[1], self.id)
    ...
    def _start(self, actor_model=None, actor_name=None):
        ...
        # Spawn actor TODO: adapt spawn
        if BaseWrapper.ID == 1:
            transform = carla.Transform(
                carla.Location(46.1, 208.9, 0.2),
                carla.Rotation(yaw=0)
            )
        else:
            transform = carla.Transform(
                carla.Location(56.1, 208.9, 0.2),
                carla.Rotation(yaw=0)
            )
        self._vehicle = self._world.spawn_actor(blueprint, transform)
    ...
    def update_state(self, frame, start_frame, timeout):
        ...
        if (self.state.collision or
            self.state.distance_to_center_line > 1.8
                or self.state.elapsed_time >= self._max_frames):
    ...
    def _get_non_sensor_data(self, frame, start_frame):
        ...
        # Elapsed time
        self.state.elapsed_time = frame - start_frame

        # Target velocity
        speed_limit = self._vehicle.get_speed_limit()
        if speed_limit:
            self.state.target_velocity = speed_limit
        else:
            self.state.target_velocity = 120
...
class ContinuousWrapper(BaseWrapper):
    ...
    def __init__(self, world, max_frames=1000, render=False):
        super(ContinuousWrapper, self).__init__(world, max_frames, render)
...
class DiscreteWrapper(BaseWrapper):
    ...
    def __init__(self, world, max_frames=1000, render=False):
        super(DiscreteWrapper, self).__init__(world, max_frames, render)
