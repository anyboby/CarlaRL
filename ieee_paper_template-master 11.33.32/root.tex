...
\paragraph{Latent space generated from rgb images} To deal with the higher input complexity in RGB-images, we extend the
encoder and decoder models to five convolutions.
       
\subsection{Training}
In order to align the algorithms with the varying input dimensions we implement a wrapper around the actual models that adjusts the network architecture accordingly. Especially because of the fact that the latent space is flattened and not 2-dimensional such as the ground truth pictures, it is necessary to implement convolutional as well as fully-connected networks. Luckily, for the stable-baselines PPO model the implementation is very straightforward. The architecture of the Keras-DDPG model however, has to be adjusted completely. 
We perform the training in four process steps, according to the previously mentioned input representations. The initial training is carried out on the ground-truth front view, then on the ground-truth bird's eye view and finally the models are trained on the latent space of RGB and ground truth segmented images. 
\subsubsection{DDPG}
The Deep Deterministic Policy Gradient had performed well on our initial attempts within Gym CarRacing, yielding reasonable hyperparameters for proper learning. The same model was applied to the new input features of CARLA but seemed to be too complex for the model. We assume, that the learning algorithm was distracted by the advanced simulation environment. The following illustration emphasizes the poor behavior of the model, while being trained under circumstances which led to good performances of other models. Hence, we continued with different learning approaches and settled on the PPO.
\begin{figure}[thpb]
	\centering
	\framebox{\parbox{3in}{
...
\subsubsection{PPO}
The stable-baselines PPO2 model is easy to train and only few hyperparameters need to be adjusted. The drawback of this approach is that customization of these pre-defined models is rather laborious. Especially a combination of diverse input types such as scalars and images requires extensive architecture adjustments. We changed the following parameters and arrived at a very fast and crisp learning behavior that enables the agent to start driving after very few episodes.
...
The learning behavior usually increases until 200k steps and starts to stagnate afterwards. The achieved episode rewards and entropy loss over several runs is very stable and performance drops occur rarely. The results on the input representations are very different even though the surroundig conditions remain the same: The PPO is trained with the same reward function and the same amount of steps at the same environment spawn point. The front-cam view results in an oscillating driving behavior and the rewards are reduced by the penalties for inaccurate delta heading. The birds-eye view on the other hand has a very smooth driving behavior and delivers the best performance in our tests. This results from the improved view on the street trajectory from above. Considering the easier and smaller latent space representation it becomes evident that the algorithm trains very fast but cannot reproduce the performance of the ground truth. We assume that the architecture does not contain the complete information in the layers that are extracted in our experiment and, hence, the latent space should usually arrive at a similar outcome as the ground truth birds-eye view.
...
Due to the simple input structure and the straight-forward reward system we achieved stable results, even when using not the most-advanced learning algorithms. We consider the PPO to be an improvement over the DDPG. Also the A3C generated promising results with average rewards around 700 and little training time. In total, these results can be explained by the simplicity of the environment. However, more time was invested on the finetuning of the hyperparameters compared with the performance in CARLA.
\subsection{CARLA}
The CARLA simulation environment is far more complex than Gym's CarRacing-v0 environment. It is necessary to implement observations, terminal conditions and the reward function, which is a very time-consuming process. The application of the stable-baselines algorithms is rather straightforward, leaving the environment itself to be the challenging part. 
\subsection{Results}
